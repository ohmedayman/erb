...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen flex bg-background">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-sidebar-bg transform transition-transform duration-200 ease-in-out lg:translate-x-0 ${
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center gap-2 px-5 h-16 border-b border-white/10">
            ...
            <button
              onClick={() => setSidebarOpen(false)}
              className="ml-auto lg:hidden text-sidebar-text hover:text-white"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
              ...
              {userMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-card rounded-xl shadow-xl border border-border py-1 z-50">
                    ...
                    <button
                      onClick={handleLogout}
                      className="flex items-center gap-2 px-4 py-2 text-sm text-red-500 hover:bg-muted transition-colors w-full"
                    >
                      <LogOut className="w-4 h-4" />
                      Logout
                    </button>
